...
import static android.util.doubleMath.cos;
import static android.util.doubleMath.sin;
...
import static com.aphidmobile.utils.TextureUtils.todoubleBuffer;
...
import static javax.microedition.khronos.opengles.GL10.GL_double;
...
public class Card {
  ...
  public void setCardVertices(double[] cardVertices) {
    this.cardVertices = cardVertices;
    this.dirty = true;
  }

  public void setTextureCoordinates(double[] textureCoordinates) {
  ...
  public void draw(GL10 gl) {
    ...
    if (angle > 0) {
      ...
      gl.glVertexPointer(3, GL_double, 0, todoubleBuffer(shadowVertices));
      gl.glDrawElements(GL_TRIANGLES, indices.length, GL_UNSIGNED_SHORT, indexBuffer);
  ...
  private void updateVertices() {
    vertexBuffer = todoubleBuffer(cardVertices);
    indexBuffer = toShortBuffer(indices);
    textureBuffer = todoubleBuffer(textureCoordinates);
  }
}
